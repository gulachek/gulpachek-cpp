			...
			for (const isDebug of tf) {
				for (const isStaticLink of tf) {
					...
					if (opts._useBuildPaths) {
						cpy.targetIncludeDir = sys.abs(Path.dest(opts.targetIncludeDir));
						cpy.targetLibDir = sys.abs(Path.dest(opts.targetLibDir));
					}

					target.dependsOn(cpp.pack(lib, {
						target: {
							platform: cpy.targetPlatform,
							includeDir: cpy.targetIncludeDir,
							libDir: cpy.targetLibDir
						},
						packageDir: cpy.packageDir
					}));
