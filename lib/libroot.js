...
class LibrootTarget extends Target {
	...
	recipe(cb) {
		...
		fs.writeFile(this.abs, JSON.stringify(this.#obj()), cb);
	}
}
...
class LibrootPackage extends Target {
	...
	constructor(cpp, lib, args) {
		...
		const sys = cpp.sys();
		const dir = Path.dest(args.packageDir || `${nameUnder}.${version}`);
		super(sys, dir);
	...
	async recipe()
	{
		const t = new Target(this.sys);
		const includeDir = this.path.join('include');

		for (const inc of this.#lib.includes()) {
			const contents = fs.readdirSync(inc.abs, { withFileTypes: true });
			for (const entry of contents) {
				const src = this.sys.src(inc.path.join(entry.name));
				if (entry.isFile())
				{
					t.dependsOn(copyFile(src, includeDir));
				}
				else if (entry.isDirectory())
				{
					t.dependsOn(copyDir(src, includeDir));
				}
				else
				{
					return Promise.reject(`${entry.name} is not a supported file type`);
				}
			}
		}
	...
	deps() {
		return [this.#libroot, ...this.#binaryFiles];
	}
}
